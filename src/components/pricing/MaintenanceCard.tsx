      ...
      <div className="p-5 bg-[#0D503C] text-[#F5F5E9]">
        <div className="flex items-center mb-3">
          ...
          <h3 className="text-xl font-bold text-[#F5F5E9] font-serif">
            {language === 'nl' ? 'Doorlopend Onderhoud & Beveiliging' : 'Ongoing Maintenance & Security'}
          </h3>
        </div>
        <div className="flex items-baseline mt-2 mb-3">
          ...
          <span className="ml-2 text-[#F5F5E9]/70">{language === 'nl' ? '/ maand + BTW' : '/ month + VAT'}</span>
        </div>
        <p className="text-[#F5F5E9]/80 mb-3 text-sm">
          {language === 'nl'
            ? 'We houden je website online, veilig en soepel draaiend — zodat jij je nergens zorgen over hoeft te maken.'
        ...
        <div className="bg-[#F5F5E9]/10 rounded-lg p-3 border border-[#F5F5E9]/20">
          <div className="flex items-center mb-2">
            ...
            <h4 className="text-sm font-semibold text-[#F5F5E9]">
              {language === 'nl' ? 'Wat is Inbegrepen' : 'What\'s Included'}
            </h4>
          </div>
          <div className="grid grid-cols-2 gap-x-2 text-[#F5F5E9]/80 text-xs">
            <div>{language === 'nl' ? '• Website hosting' : '• Website hosting'}</div>
            <div>{language === 'nl' ? '• Beveiligingsmonitoring' : '• Security monitoring'}</div>
            <div>{language === 'nl' ? '• Regelmatige software-updates' : '• Regular software updates'}</div>
            <div>{language === 'nl' ? '• Prestatie-optimalisatie' : '• Performance optimization'}</div>
          </div>
        </div>
      </div>
    </div>
