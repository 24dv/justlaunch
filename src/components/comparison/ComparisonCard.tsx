...
const ComparisonCard: React.FC<ComparisonCardProps> = ({ title, data, highlight = false }) => {
  ...
  return (
    <Card className={`mb-6 overflow-hidden transition-all duration-200 ${highlight ? 'border-[#0D503C] ring-1 ring-[#0D503C]/30 shadow-lg' : ''} bg-white`}>
      <CardHeader className={`pb-2 ${highlight ? 'bg-[#0D503C] text-[#F5F5E9]' : 'bg-[#F5F5E9]'}`}>
